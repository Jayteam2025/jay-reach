...
**Endpoint** `apps/web/app/api/webhooks/smartlead/route.ts` : POST, `runtime nodejs`. Auth par **`?org=&token=`** — token comparé en **temps constant** (`timingSafeEqual`) au secret `credentials.config.webhook_secret` du provider Smartlead de l'org. (Choix : Smartlead ne permet de configurer qu'une **URL** de webhook, pas d'en-tête personnalisé → le secret vit dans l'URL, comme le pattern `extension/linkedin/update`.) Body parsé (Zod) → `processSmartleadEvent`.
...
- **Conditions d'étape — sélecteur RETIRÉ de l'UI [retour de review #24]** : le sélecteur `previous_opened` / `previous_accepted` / `no_reply` a été retiré de l'écran. Motif (JB) : il était **enregistré mais jamais évalué** par le tick → l'interface promettait un comportement (« envoyer seulement si… ») qu'elle ne tenait pas, cas dangereux avec `no_reply` (relance envoyée à ceux qui **ont** répondu → désinscriptions, réputation domaine). Or les trois conditions dépendent de données **pas encore captées** (ouvertures = T27 webhooks, acceptations LinkedIn, réponses) → l'évaluation dans `composeTick` ne peut pas se faire maintenant. On applique l'option recommandée : **retirer le réglage sans effet** ; la colonne `conditions` et le paramètre des actions restent, le moteur (évaluation `composeTick` → `skipped`) viendra avec la captation des retours. L'édition inline du délai a aussi été déplacée dans la modale (persistée) ; le stepper « live » de la maquette est retiré.

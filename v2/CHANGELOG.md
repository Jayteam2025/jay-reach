...
### Ajouté
- **Scoring LLM des signaux + auto-apprentissage de la blacklist (T12)** : nouvelle file `signals.score` + handler `runScore` — pré-filtre cabinets (blacklist DB + NAF) et signaux périmés, scoring par persona (prompt `personas.scoring_prompt`), persistance `score/status`, et auto-apprentissage (`learnRecruitmentAgency`) quand le modèle met 0 avec un motif « cabinet ». Modèle derrière une interface `SignalScorer` (adaptateur Anthropic réel `scorer-anthropic.ts`, provider `anthropic` via coffre + repli env) ; testé hermétiquement (scorer déterministe) et vérifié en réel sur la base hébergée (transaction annulée). Producteur périodique `enqueueScoringForOrgs`.
